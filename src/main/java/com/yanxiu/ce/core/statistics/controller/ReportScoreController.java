...
import com.yanxiu.ce.core.statistics.enums.ReportScoreWayEnum;
 ...
 */
@Controller
@RequestMapping("/core/statistics/score")
public class ReportScoreController{
	...
	@RequestMapping("main")
	public String main(ReportScoreQuery query,Model model){
		if(StringUtils.isBlank(query.getWay())){
			query.setWay(ReportScoreWayEnum.NUMBERS.getValue()+"");
		}
...
//		}
		...
		List<?> ways = ReportScoreWayEnum.toList();
		model.addAttribute("ways", ways);
		model.addAttribute("query", query);
		
		String wayName = "";//副标题
		if(StringUtils.isNotBlank(query.getWay())){
			wayName = ReportScoreWayEnum.getDesc(Integer.parseInt(query.getWay()));
		}else{
			wayName = ReportScoreWayEnum.NUMBERS.getDesc();
		}
		...
		String groupByProName = ReportScoreWayEnum.getName(Integer.parseInt(query.getWay()));
		//如果是人数分布
		if(query.getWay().equals(ReportScoreWayEnum.NUMBERS.getValue()+"")){
...
//		else if(query.getWay().equals(ReportScoreWayEnum.STAGE.getValue()+"")){
//			iname = "name";
//			tableName="t_conf_stage";
//			groupByProName = "stage";
//		}else if(query.getWay().equals(ReportScoreWayEnum.COURSE.getValue()+"")){
//			iname = "name";
//			tableName="t_conf_course";
//			groupByProName = "course";
//		}
		List<NameValue> results = reportScoreService.groupByProperty(iname,groupByProName, tableName,joinOnName,
		...
		return "core/statistics/score/main";
	}
	 ...
	 * @return
	 */
	private ReportScoreQuery configPstsQuery(ReportScoreQuery query) {
