...
<script src="${ctx}/static/js/sha256.js"></script>
...
body {
    background: url(${ctx}/static/images/loginbg_01.jpg) no-repeat center center fixed;
...
a:link {color: #285e8e;}
.main_box {
    position: absolute; top:50%; left:50%; margin-top:-260px; margin-left: -300px; padding: 30px; width:600px; height:400px;
...
.login_error {border:1px #ff0000 solid; -moz-box-shadow:0px 0px 5px #ff0000; -webkit-box-shadow:0px 0px 5px #ff0000; box-shadow:0px 0px 5px #ff0000;}
...
function submitLogin(){
	...
	$(this).find('.in').each(function(i){
		if ($.trim($(this).val()).length == 0) {
			...
			$(this).css({"border":"1px #ff0000 solid","-moz-box-shadow":"0px 0px 5px #ff0000","-webkit-box-shadow":"0px 0px 5px #ff0000","box-shadow":"0px 0px 5px #ff0000"});
			issubmit = false;
			if (i_index == 0)
				i_index  = i;
		}
    ...
    passport.init({
        appKey: "a21849e4-e04e-4bae-949b-44fa4072f311"
    });
...
function choose_bg() {
	...
	$('body').css('background-image', 'url(${ctx}/static/images/loginbg_0'+ bg +'.jpg)');
}
</script>
</head>
<body>
...
<div class="main_box">
	...
	<div class="login_box">
        ...
        <div class="login_form">
    		...
    		<form action="${ctx}/login" id="login_form" method="post"  onSubmit="return submitLogin();">
                <input type="hidden" name="jfinal_token" value="${jfinal_token }" />
    			<div class="form-group">
    				<label for="j_username" class="t">用户名：</label> <input id="j_username" value="${loginName }" name="loginName" type="text" class="form-control x319 in" autocomplete="off">
    			</div>
    			<div class="form-group">
    				<label for="j_password" class="t">密　码：</label> <input id="j_password" value="" name="loginPwd" type="password" class="form-control x319 in">
    			</div>
    			<!--  <div class="form-group">
    				<label for="j_captcha" class="t">验证码：</label> <input id="j_captcha" name="captcha" type="text" class="form-control x164 in">
    				<img id="captcha_img" alt="点击更换" title="点击更换" src="${ctx}/static/images/captcha.jpeg" class="m">
    			</div>-->
    			<div class="form-group">
                    <label class="t"></label>
                    <label for="j_remember" class="m"><input id="j_remember" name="rememberMe" type="checkbox" value="true">&nbsp;记住登录账号!</label>
    			</div>
	...
	<div class="bottom">COPYRIGHT © 2008-2016 ALL RIGHTS RESERVED <a target="_blank" href="http://www.teacherhubei.com">湖北教师发展网 </a></div>
</div>
</body>
</html>