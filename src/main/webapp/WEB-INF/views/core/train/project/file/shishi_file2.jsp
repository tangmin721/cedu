...
function checkupload2file(obj){
	...
	function upload2(obj){
		if(checkupload2file(obj)){
			$("#file2_form").bjuiajax('ajaxForm', {
   				callback:function(data){
   					if(data.statusCode==300){
   						...
   						if(data.data.fileId!="empt"){
   							...
   							$("#file2_span").removeClass("label-default").addClass("label-primary");
   							$("#file2_span").text("已上传");
   							
   							$("#file2_status_span").removeClass("label-default").addClass("label-primary");
   							$("#file2_status_span").text("待审核");
   							
   							
   							var againUp = '<div class="file-box-again">'
   								+'<form id="file2_form" action="${ctx }/core/train/project/file/upload2" method="post" enctype="multipart/form-data">	'
   								+'<input type="hidden" name="pid" value="${pid }"/>'
   								+'<input type="hidden" name="orgId" value="${orgId }"/>'
   								+'<input type="file" name="file" class="file-again" size="28" onchange="upload2(this);" id="upload2_file" />'
   							...
   							var hadUp = '<a target="_blank" href="'+data.data.downloadUrl+'" class="btn btn-blue">下载</a>'
   									+'<button type="button" class="btn btn-red" onclick="deleteFile2('+data.data.projectFileId+')">删除</button>';
   									
   							var checkStatus	='<span class="label label-primary">待审核</span>';		
   									
   							//上传栏		
   							$("#file2_td_upload").html(againUp);
   							//状态栏
   							$("#file2_td_status").html(checkStatus);
   							//操作栏
   							$("#file2_td_oprate").html(hadUp);
	...
	function deleteFile2(projectFileId){
		$.ajax({ 
	        type: "POST", 
	        url: "${ctx}/core/train/project/file/deleteFile2/"+projectFileId,
	        dataType : "json",
	        success: function(data) {
	        	$(this).alertmsg('ok', "删除成功");
	        	$("#file2_span").removeClass("label-primary").addClass("label-default");
				$("#file2_span").text("未上传");
				
				$("#file2_status_span").removeClass("label-primary").addClass("label-default");
				$("#file2_status_span").text("");
				
				var upstr = '<div id="file2_btn_group" class="btn-group pull-left" role="group">'
	   			+'<div class="file-box">'
	   			+'<form id="file2_form" action="${ctx }/core/train/project/file/upload2" method="post" enctype="multipart/form-data">'
	   			+'<input type="hidden" name="pid" value="${pid }"/>'
	   			+'		<input type="hidden" name="orgId" value="${orgId }"/>'
	   			+'		<input type="file" name="file" class="file" size="28" onchange="upload2(this);" id="upload2_file" />'
				...
				//上传栏		
				$("#file2_td_upload").html(upstr);
				//状态栏
				$("#file2_td_status").html("");
				//操作栏
				$("#file2_td_oprate").html("");
	        },
	        error :  function(){
				   ...
				   <c:otherwise>  
	                  	...
	                  	<td id="file2_td_oprate">
	                  		<div id="file2_btn_group" class="btn-group pull-left" role="group">
									...
									<button type="button" class="btn btn-red" onclick="deleteFile2(${objFile2.id })">删除</button>
							</div>
	                  	</td>
				   </c:otherwise>
