...
function applyCheck(id){
	$(this).dialog({id:'project_roster_check_form', url:'${ctx}/core/train/project/roster/checkForm/'+id, type:'POST',title:'报名审批',mask:'true',width:'600',height:'520'});
}
...
$('#but_check_roster').on('click', function(e) {
	 $(this).dialog({id:'check-fr-dialog', url:'${ctx}/core/train/project/roster/checkStaticsFr/${pid}', title:'上报情况统计',mask:'true',width:'850',height:'600'});
});
...
<div class="bjui-pageHeader">
	<div class="btn-group pull-left" role="group">
		<button type="button" id="but_check_roster" class="btn btn-blue" data-icon="edit" >上报情况统计</button>
	</div>
    <form id="pagerForm" class="project_roster_apply_pager_form" data-toggle="ajaxsearch" action="${ctx}/core/train/project/roster/checkList/${pid}" method="post" data-reload-navtab="true">
        ...
        <div class="bjui-searchBar pull-right">
            ...
            <button type="submit" class="btn btn-blue" id="check_audit_search" data-icon="search">查询</button>&nbsp;
        </div>
    ...
    <table data-toggle="tablefixed" data-width="100%">
    	...
    	<tr data-id="${item.id }">
			...
			<td>
				...
				<c:if test="${item.status ==1}"><span class="label label-info">待审批</span></c:if>
				...
				<c:if test="${item.status ==4}"><span class="label label-info">待审批</span></c:if>
			</td>
			<td>
				<c:if test="${item.status==1 || item.status==4}">
					<button type="button" class="btn btn-purple" data-icon="check-square-o" onclick="applyCheck(${item.id})" data-toggle="tooltip"  data-placement="top" title="审核"></button>
