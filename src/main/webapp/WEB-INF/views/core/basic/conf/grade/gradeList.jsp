...
$('#grade_table').on('ifChanged','input[name="ids"]', function(e) {
	e.stopPropagation();
...
$('#grade_table').on('click','tr:not(.trth)', function(e) {
	e.stopPropagation();
...
$('#but_grade_add').on('click', function(e) {
	$(this).dialog({id:'grade_form', url:'${ctx}/core/basic/conf/grade/gradeForm', type:'POST',title:'添加年级',mask:'true',width:'420',height:'260'});
});

$('#but_grade_edit').on('click', function(e) {
	var list = grade.selectedIds();
	if(list.length==0){
		...
		var id = list[0];
		$(this).dialog({id:'grade_form', url:'${ctx}/core/basic/conf/grade/gradeForm/'+id, type:'POST',title:'编辑年级',mask:'true',width:'460',height:'260'});
	}
});

$('#but_grade_delete').on('click', function(e) {
	var list = grade.selectedIds();
	if(list.length==0){
		$(this).alertmsg('warn', '请选择需要删除的行！');
	}else{
		$(this).bjuiajax('doAjaxChecked', {url:'${ctx}/core/basic/conf/grade/deleteGradeByIds',idname:"ids",group:"ids",confirmMsg:"确定要删除选中项吗？",callback:"delRefushGradeTable"});
	}
});

function delRefushGradeTable(json){
...
var grade = {
		...
		selectedIds: function(){
			var list = [];
			$('#grade_table input[name="ids"]').each(function(){ 
				 var checked = $(this).is(':checked'), val = $(this).val();
				 if(checked){
					 list.push(val);
				 }
...
<div class="bjui-pageHeader">
	<div class="btn-group pull-left" role="group">
		<shiro:hasPermission name="Grade:create">
			<button type="button" id="but_grade_add" class="btn btn-blue" data-icon="plus-circle" >添加</button>
		</shiro:hasPermission>
		<shiro:hasPermission name="Grade:update">
			<button type="button" id="but_grade_edit" class="btn btn-green" data-icon="edit" >编辑</button>
		</shiro:hasPermission>
		<shiro:hasPermission name="Grade:delete">
			<button type="button" id="but_grade_delete" class="btn btn-red" data-icon="times-circle" >删除</button>
		</shiro:hasPermission>
	</div>
    <form id="pagerForm" class="grade_pager_form" data-toggle="ajaxsearch" action="${ctx}/core/basic/conf/grade/gradeList" method="post" data-reload-navtab="true">
...
<div id="grade_table" class="bjui-pageContent tableContent">
