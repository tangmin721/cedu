...
$('#teacher_move_recode_out_table').on('ifChanged','input[name="ids"]', function(e) {
	e.stopPropagation();
...
$('#teacher_move_recode_out_table').on('click','tr:not(.trth)', function(e) {
	e.stopPropagation();
...
$('#but_teacher_move_recode_out_add').on('click', function(e) {
	$(this).dialog({id:'teacher_move_recode_out_form', url:'${ctx}/core/basic/teacher/move/recode/teacherMoveRecodeForm', type:'POST',title:'添加教师调动',mask:'true',width:'600',height:'520'});
});

$('#but_teacher_move_recode_out_edit').on('click', function(e) {
	...
	if(list.length==0){
		...
		var id = list[0];
		$(this).dialog({id:'teacher_move_recode_out_form', url:'${ctx}/core/basic/teacher/move/recode/teacherMoveRecodeForm/'+id, type:'POST',title:'编辑教师调动',mask:'true',width:'600',height:'520'});
	}
});

$('#but_teacher_move_recode_out_delete').on('click', function(e) {
	var list = teacherMoveRecode.selectedIds();
	if(list.length==0){
...
function cancelMove(id){
	$.ajax({
		type: "POST",
		url: '${ctx}/core/basic/teacher/move/recode/cancelMove/'+id,
		dataType : "json",
...
function delRefushTeacherMoveRecodeTable(json){
	$('.teacher_move_recode_out_pager_form').bjuiajax('doSearch', {url:'${ctx}/core/basic/teacher/move/recode/teacherMoveRecodeList'});
}
...
var teacherMoveRecode = {
		...
		selectedIds: function(){
			var list = [];
			$('#teacher_move_recode_out_table input[name="ids"]').each(function(){ 
				 var checked = $(this).is(':checked'), val = $(this).val();
				 if(checked){
					 list.push(val);
				 }
...
<div class="bjui-pageHeader">
	<div class="btn-group pull-left" role="group">
		<shiro:hasPermission name="TeacherMoveRecode:create">
			<button type="button" id="but_teacher_move_recode_out_add" class="btn btn-blue" data-icon="plus-circle" >添加</button>
		</shiro:hasPermission>
		<shiro:hasPermission name="TeacherMoveRecode:update">
			<button type="button" id="but_teacher_move_recode_out_edit" class="btn btn-green" data-icon="edit" >编辑</button>
		</shiro:hasPermission>
		<shiro:hasPermission name="TeacherMoveRecode:delete">
			<button type="button" id="but_teacher_move_recode_out_delete" class="btn btn-red" data-icon="times-circle" >删除</button>
		</shiro:hasPermission>
	</div>
    <form id="pagerForm" class="teacher_move_recode_out_pager_form" data-toggle="ajaxsearch" action="${ctx}/core/basic/teacher/move/recode/schoolOutList" method="post" data-reload-navtab="true">
...
<div id="teacher_move_recode_out_table" class="bjui-pageContent tableContent">
    <table data-toggle="tablefixed" data-width="100%">
    	...
    	<tr class="trth">
			...
			<th align="center" data-order-field="sendDate" title="申请调动时间">申请调动时间</th>
			<th align="center" title="待调入学校">待调入学校</th>
			<th align="center" title="调出状态">调出状态</th>
			<th align="center" data-order-field="sendDate" title="调出时间">调出时间</th>
    	...
    	<tr data-id="${item.id }">
			...
			<td>${item.tprovinceName }${item.tcityName }${item.ttownName }${item.tschoolName }</td>
			<td>
			...
			<td>
				<c:if test="${item.moveStatus ==1}">
					<button type="button" class="btn btn-danger" data-icon="times-circle" onclick="cancelMove(${item.id })" >取消调动</button>
				</c:if>
			</td>
    	</tr>
    	</c:forEach>
