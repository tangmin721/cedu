...
$('#teacher_move_recode_all_table').on('ifChanged','input[name="ids"]', function(e) {
	e.stopPropagation();
...
$('#teacher_move_recode_all_table').on('click','tr:not(.trth)', function(e) {
	e.stopPropagation();
...
$('#but_teacher_move_recode_all_add').on('click', function(e) {
	$(this).dialog({id:'teacher_move_recode_all_form', url:'${ctx}/core/basic/teacher/move/recode/teacherMoveRecodeForm', type:'POST',title:'添加教师调动',mask:'true',width:'600',height:'520'});
});

$('#but_teacher_move_recode_all_edit').on('click', function(e) {
	...
	if(list.length==0){
		...
		var id = list[0];
		$(this).dialog({id:'teacher_move_recode_all_form', url:'${ctx}/core/basic/teacher/move/recode/teacherMoveRecodeForm/'+id, type:'POST',title:'编辑教师调动',mask:'true',width:'600',height:'520'});
	}
});

$('#but_teacher_move_recode_all_delete').on('click', function(e) {
	var list = teacherMoveRecode.selectedIds();
	if(list.length==0){
...
var teacherMoveRecode = {
		...
		selectedIds: function(){
			var list = [];
			$('#teacher_move_recode_all_table input[name="ids"]').each(function(){ 
				 var checked = $(this).is(':checked'), val = $(this).val();
				 if(checked){
					 list.push(val);
				 }
...
<div class="bjui-pageHeader">
    ...
    <form id="pagerForm" class="teacher_move_recode_all_pager_form" data-toggle="ajaxsearch" action="${ctx}/core/basic/teacher/move/recode/pctAllList" method="post" data-reload-navtab="true">
...
<div id="teacher_move_recode_all_table" class="bjui-pageContent tableContent">
    <table data-toggle="tablefixed" data-width="100%">
    	...
    	<tr class="trth">
			...
			<th align="center" title="人员类型">人员类型</th>
			<th align="center" data-order-field="sendDate" title="申请时间">申请时间</th>
			<th align="center" title="调出校">调出校</th>
			<th align="center" title="调入校">调入校</th>
			<th align="center" title="调动状态">调动状态</th>
