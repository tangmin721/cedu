...
$('#province_check_score_table').on('ifChanged','input[name="ids"]', function(e) {
	e.stopPropagation();
...
$('#province_check_score_table').on('click','tr:not(.trth)', function(e) {
	e.stopPropagation();
...
function checkTown(city,town,cityName,townName){
	$(this).dialog({id:'np-check-town', url:'${ctx}/core/score/checkScoreList/0?pcity='+city+'&ptown='+town, type:'POST',title:'学时申报明细审核——'+cityName+townName,mask:'true',width:'1500',height:'640'});
} 
...
var score = {
		...
		selectedIds: function(){
			var list = [];
			$('#province_check_score_table input[name="ids"]').each(function(){ 
				 var checked = $(this).is(':checked'), val = $(this).val();
				 if(checked){
					 list.push(val);
				 }
...
$('#but_score_allpass_province').on('click',function(e){
...
<div class="bjui-pageHeader">
	<div class="btn-group pull-left" role="group">
		<button type="button" id="but_score_allpass_province" class="btn btn-success" data-icon="keyboard-o" >一键全部通过</button>
	</div>
    <form id="pagerForm" class="score_pager_form" data-toggle="ajaxsearch" action="${ctx}/core/scoreJoin/provinceJoinCheckScoreList" method="post" data-reload-navtab="true">
        ...
        <div class="bjui-searchBar pull-right">
        	<shiro:hasAnyRoles name="SUP_ADMIN,SYS_ADMIN,PROVINCE_ADMIN,CITY_ADMIN,TOWN_ADMIN,SCHOOL_ADMIN">
            	<div <c:if test="${sessionScope.currentUser.province==0}">style="display: inline;"</c:if><c:if test="${sessionScope.currentUser.province!=0}">  style="display: none;" </c:if>>
             	...
             	data-nextselect="#pro_join__sform_city"  data-refurl="${ctx}/system/area/city/citys?province={value}" data-size="20" data-width="140px">
        		...
        		<select name="city" id="pro_join__sform_city" data-toggle="selectpicker" 
        			 data-nextselect="#pro_join__sform_town" 
	            ...
	            <select name="town" id="pro_join__sform_town" data-toggle="selectpicker" 
	            	data-nextselect="#pro_join__sform_school" 
	            ...
	            <select name="school" id="pro_join__sform_school" data-toggle="selectpicker" 
            ...
            <button type="submit" class="btn btn-blue" data-icon="search" id="province_check_score_list_search">查询</button>&nbsp;
...
<div id="province_check_score_table" class="bjui-pageContent tableContent">
    <table data-toggle="tablefixed" data-width="100%">
    	...
    	<tr data-id="${item.id }">
    		...
    		<td>
    			<button type="button" class="btn btn-success" data-icon="check-circle" onclick="checkTown(${item.city},${item.town},'${item.cityName}','${item.townName}')" >明细审核</button>
    		</td>
    	</tr>
    	</c:forEach>
    	</tbody>
